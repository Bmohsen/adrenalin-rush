...
namespace Engine {
	...
	void Player::updateControl(float dt) {
		...
		if (wheel > 0) zoomIn(zoomSpeed);
		else if (wheel < 0) zoomOut(zoomSpeed);
		handleMovement(dt);
	}

	void Player::zoomIn(float amount){
		camera.zoom += amount;
		if (camera.zoom > maxZoomIn) camera.zoom = maxZoomIn;
	}
	void Player::zoomOut(float amount){
		camera.zoom -= amount;
		if (camera.zoom < maxZoomOut) camera.zoom = maxZoomOut;
	}
